...
void PlotAdapter::set_type(const PlotType& type) { 
    _plot->set_type(type); 
    emit type_changed(_plot->id(), type); 
}
...
int PlotAdapter::features_count() const {
...
unsigned int PlotAdapter::plot_type_count() {
    return Plot::__static__plots_types_indexes__.size();
}
QString PlotAdapter::plot_type_label(unsigned int idx) {
