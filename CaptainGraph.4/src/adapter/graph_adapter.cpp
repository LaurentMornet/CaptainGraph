...
short GraphAdapter::plots_count() const { return _graph->plots_count(); }
Plot& GraphAdapter::plot(unsigned int idx) { return _graph->plot(idx); }
PlotType GraphAdapter::plot_type(unsigned int idx) const { return _graph->plot(idx).type(); }
