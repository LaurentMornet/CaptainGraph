 ...
 * Accesseurs simples
 */
LayoutType Layout::type() const { return _type; }
void Layout::set_type(const LayoutType type) { _type = type; }
...
void Layout::set_weight(double new_weight) {
    ...
    double to_deploy = new_weight - initial_weight;
    int how_many = _ancestor->_sublayouts.size() - idx + 1;
    for(size_t i=idx+1; i<_ancestor->_sublayouts.size(); i++) _ancestor->_weights[idx] += (to_deploy / how_many);
}
/**
...
std::map<std::string, double[4]> Layout::nodes_bounds() {
    std::map<std::string, double[4]> map;
    nodes_bounds(map, 0.0, 0.0, 1.0, 1.0);
    return map;
}
void Layout::nodes_bounds(std::map<std::string, double[4]>& buffer, double x, double y, double width, double height) {
    if(_type == LayoutType::Node) {
        ...
        for (size_t i = 0; i < _sublayouts.size(); ++i) {
            ...
            if (_type == LayoutType::Horizontal) {
                ...
                _sublayouts[i]->nodes_bounds(buffer, x + offset * width, y, w, height);
                offset += ratio;
            } else { // VERTICAL
                double h = height * ratio;
                _sublayouts[i]->nodes_bounds(buffer, x, y + offset * height, width, h);
                offset += ratio;
            }
        }
    }
 ...
 */
// int main() {
//     Layout layout(nullptr, LayoutType::Node);
//     layout.get("A")->get().split(LayoutType::Horizontal);
//     layout.get("B")->get().split(LayoutType::Vertical);
//     layout.get("B")->get().split(LayoutType::Horizontal);
//     layout.get("A")->get().split(LayoutType::Horizontal);
//     layout.get("E")->get().split(LayoutType::Vertical);
//     layout.get("F")->get().split(LayoutType::Vertical);
//     layout.get("E")->get().merge("F");
//     layout.get("B")->get().merge("D");
//     layout.get("G")->get().merge("E");
//     layout.draw_html("layout_ids.htm", 150, 80);
//     return 0;
// }
